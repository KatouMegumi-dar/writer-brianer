...
(function () {
    ...
    async function runCabinetDiscussion({
        ...
        const endpoints = Array.isArray(config?.selectiveMode?.apiEndpoints)
            ? config.selectiveMode.apiEndpoints.filter(ep => ep && ep.enabled !== false)
            : [];
        ...
        const fallbackBooks = Array.from(new Set(endpoints.flatMap(ep => {
            if (Array.isArray(ep.worldBooks)) return ep.worldBooks;
            return ep.worldBook ? [ep.worldBook] : [];
        }).filter(Boolean)));
            ...
            for (const endpoint of endpointsToProcess) {
                ...
                effectiveBooks.forEach(wb => {
                    ...
                    if (!Array.isArray(assigned)) {
                        assigned = (endpoint.worldBook === wb && legacyEntries.length > 0) ? legacyEntries : [];
                    }
