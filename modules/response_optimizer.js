...
(function () {
    ...
    function getConfig() {
        ...
        if (!Number.isFinite(Number(merged.worldbookMaxChars))) merged.worldbookMaxChars = getDefaultConfig().worldbookMaxChars;
        return merged;
    }
        ...
        try {
            ...
            if (shouldInjectWorldbook === undefined) {
                if (!targetTag) {
                    ...
                    const extracted = extractContentByTag(content, targetTag);
                    shouldInjectWorldbook = !!(extracted && extracted.trim());
                }
            }
            const worldbookText = shouldInjectWorldbook
                ? await buildWorldbookReferenceText(config.worldBooks, config.worldbookMaxChars, config.assignedEntriesMap)
                : '';
            ...
            if (targetTag) {
                const extractedFromResponse = extractContentByTag(rawResponse, targetTag);
                if (extractedFromResponse && extractedFromResponse.trim()) {
                    optimizedContent = extractedFromResponse.trim();
                    Logger.log(`[正文优化] 从返回内容中提取到 <${targetTag}> 标签内容，长度: ${optimizedContent.length}`);
                } else {
                    // 如果没有找到标签，使用清理后的原始返回
                    Logger.log(`[正文优化] 返回内容中未找到 <${targetTag}> 标签，使用清理后的内容`);
    ...
    async function buildWorldbookReferenceText(worldBooks, maxChars, assignedEntriesMap) {
        const names = Array.isArray(worldBooks) ? Array.from(new Set(worldBooks.filter(Boolean))) : [];
        const limit = Number.isFinite(Number(maxChars)) ? Math.max(0, Math.min(20000, Number(maxChars))) : 0;
        ...
        for (const name of names) {
            try {
                ...
                const selectedIds = Array.isArray(entriesMap?.[name]) ? entriesMap[name] : [];
                const filteredPairs = selectedIds.length > 0
                    ? enabledPairs.filter(([uid]) => selectedIds.includes(uid))
                    : enabledPairs;
                if (filteredPairs.length === 0) continue;

                const header = `[世界书：${name}]\n`;
                if (current + header.length > limit) break;
                output += header;
...
回答要简洁明了。`;
    ...
    function revertToOriginal() {
        ...
        if (messageId !== null && original) {
            updateMessageContent(messageId, original);
            if (window.toastr) {
