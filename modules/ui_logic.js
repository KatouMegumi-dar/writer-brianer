...
(function () {
    ...
    function ensureResponseOptimizerConfig(config) {
        ...
        if (!Number.isFinite(Number(config.responseOptimizer.worldbookMaxChars))) {
            config.responseOptimizer.worldbookMaxChars = 2000;
        }
    ...
    function loadResponseOptimizerSettingsToUI() {
        ...
        const wbMaxEl = document.getElementById('wbap-resp-opt-worldbook-max-chars');
        if (wbMaxEl) wbMaxEl.value = Number.isFinite(Number(respOptCfg.worldbookMaxChars)) ? respOptCfg.worldbookMaxChars : 2000;
    ...
    function saveResponseOptimizerSettings(showToast = true) {
        ...
        const worldbookMaxChars = parseInt(document.getElementById('wbap-resp-opt-worldbook-max-chars')?.value, 10);
        ...
        respOptCfg.worldbookMaxChars = Number.isFinite(worldbookMaxChars) ? Math.max(0, Math.min(20000, worldbookMaxChars)) : (respOptCfg.worldbookMaxChars || 2000);
        ...
        if (WBAP.CharacterManager?.isTemporaryConfig && WBAP.mainConfig?.characterConfigs?.default) {
            ...
            defaultRespOptCfg.worldbookMaxChars = respOptCfg.worldbookMaxChars;
    ...
    function initSuperMemoryUI() {
        ...
        const agentInputs = ['wbap-agent-endpoint-select', 'wbap-agent-model', 'wbap-agent-system-prompt'];
    ...
    function renderSuperMemoryAgentConfig(agentKey) {
        ...
        document.getElementById('wbap-agent-model').value = config.model || '';
    ...
    function loadGraphSettings() {
        ...
        if (modelEl) modelEl.value = config.graphUpdateModel || '';
    ...
    async function updatePedsaRustStatus() {
        ...
        if (WBAP.PedsaClient) {
            try {
                const isHealthy = await WBAP.PedsaClient.checkHealth();
                if (isHealthy) {
