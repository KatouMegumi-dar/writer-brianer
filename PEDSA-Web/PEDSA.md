...
| 指标 | V2 (先前架构) | V3 (SoA + mmap) | V3.5 (Chaos Optimized) | 备注 |
| :--- | :--- | :--- | :--- | :--- |
| **总节点数** | 10,000,000 | 10,000,000+ | 10,000,000+ | 包含磁盘数据与热插入缓冲区 |
| **内存占用** | ~3.2 GB | **~320 MB** | **~380 MB** | 增加 Chaos SoA 存储空间 |
| **单次全量扫描耗时** | ~20 ms | **7.6 ms (SIMD)** | **132 ms (Chaos)** | 混沌检索全量扫描 (10M 规模) |
| **混沌检索加速比** | N/A | 291 ms (Linear) | **132 ms (Two-stage)** | **2.2x 性能提升** |
| **热插入支持** | 不支持 | **支持** | **支持** | 无需重启即可实现增量更新 |
...
*   **Chaos Store (混沌存储)**：在 V3.5 中，混沌指纹 (128-bit) 与混沌向量 (f16) 也被重构为 SoA 布局。通过将 `fingerprints` 和 `vectors` 在内存中连续存放，显著提升了线性扫描时的缓存局部性。
...
1.  **粗排 (Coarse Rank - 1-bit Quantization)**：利用 128-bit 混沌指纹（1-bit 量化后的特征）进行 Hamming 距离计算（XOR + popcount）。在 10M 规模下，初步筛选出距离 < 64 的候选节点。
2.  **精排 (Fine Rank - f16 Cosine Similarity)**：对候选节点进行 f16 维度的余弦相似度精确计算。
...
*   其实，我个人是很欣赏奥姆剃刀理论的：如无必要，勿增实体；“少即是好，易即是难”，我觉得这才是 Coding 的美学。
*   这玩意实际上就是一个 Graph 引擎，检索到什么东西，完全取决于图构建得怎么样，所以还是挺不通用的。不过，根据大家的反馈，单就用在 AIRP 的记忆检索上，效果还是挺不错的就是啦。
*   实际上，我最开始想用 MOJO 来做 GPU 加速来着，但 MOJO 现在对 WASM 和 Windows 的支持还是太差了，最后决定还是用RUST来写，MOJO、GPU 什么的以后再说吧！